...
```{r Para la media, fig.height = 3 , fig.width = 8}
library(ggplot2)
mu = 23.20
sig = 4.5
n = 40
...
regionX=seq(zc,zt,0.01)            # Intervalo a sombrear
xP <- c(zc,regionX,zt)             # Base de los polígonos que crean el efecto "sombra"
yP <- c(0,dnorm(regionX,0,1),0) 
g <- curve(dnorm(x,0,1),xlim=c(-4,4),yaxs="i",ylim=c(0,0.43),xlab = NULL, ylab=NULL)
...
$\displaystyle Z_c = \frac{{\bar X-\mu }}{{\sigma /\sqrt n }} = `r zc`$\\
